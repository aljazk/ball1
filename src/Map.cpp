...
void Map::drawMap(sf::RenderWindow &window){//make one map.texture for all objects so you can use one draw
    ...
    wallObject.resize(windowSize.x/100*windowSize.y/100*4);
    int f = 0;
    for(unsigned int x = 0; x < windowSize.x/100; x++){
        for(unsigned int y = 0; y < windowSize.y/100; y++){
...
void Map::loadMap(string fileName, int wx, int wy){
    ...
    unsigned int windowWidth = wx;
    unsigned int windowHeight = wy;
...
vector<sf::Vector2i> Map::getWalls(){
	vector<sf::Vector2i> wall;
	for(unsigned int i=0; i<wallx.size(); i++){
		sf::Vector2i newWall(wallx[i], wally[i]);
