...
void Ball::drawBall(sf::RenderTarget &window){
    ballSprite.setPosition(pos.x-25,pos.y-25);
    ...
    if (!lx && !ly){
        ...
        for (int j = 0; j < lights; j++){
            ...
            light[joints-1].position = sf::Vector2f(pos.x+25*xbase,pos.y+25*ybase);
            light[joints-1].color = sf::Color::Transparent;
        }
    } else {
        if (lights > 50)
            lights = 50;
        float dist = sqrt(pow(pos.x-lx, 2)+pow(pos.y-ly, 2));
        int joints = (dist)/4;
        if (joints < 3)
            joints = 3;
        lightning.resize(joints*lights);
        for (int j=0; j < lights; j++){
            float f = atan2(pos.y - ly, pos.x - lx);
            f = f*180/MATH_PI;
            sf::Vertex* light = &lightning[j*joints];
            light[0].position = sf::Vector2f(pos.x,pos.y);
            light[0].color = sf::Color::Transparent;
            light[1].position = sf::Vector2f(pos.x,pos.y);
            light[1].color = sf::Color(255,255,51,30);
            float ang = f+180;
            if (ang > 360) ang -= 360;
            ang *= MATH_PI/180;
...
void Ball::makeLightning(sf::RenderTarget &window){
    if (mouseButton && power > 0){
        power -= (float)1/fps;
        float dist = sqrt(pow(pos.x-mousex, 2)+pow(pos.y-mousey, 2));
        int joints = (dist)/10;
        lightning.resize(joints);
        float f = atan2(pos.y - mousey, pos.x - mousex);
        f = f*180/MATH_PI;
